...
- `data`：定义数据集契约、provider 抽象、文件导入、标准化、UTC 对齐、会话标注、质量检查、Bronze / Silver / Gold 写入
...
1. 外部历史文件先进入 `data` 模块，写入 `Bronze`
2. 标准化、UTC 对齐、会话标注和质量检查后产出 `Silver`
3. 研究输入、forward returns、因子输入矩阵和回测产物写入 `Gold`
4. `research` 与 `factors` 基于 `Gold` 产出验证报告和 tearsheet
5. `backtest` 基于因子结果和策略定义输出研究回测结果与订单级事件骨架
6. `registry` 记录 dataset / factor / strategy 的版本、状态和引用关系
7. `runtime` 组合注册表状态和运行时上下文，返回 Deploy Gate / Runtime Gate 结果
8. `Notebook`、`CLI`、`FastAPI` 作为统一入口消费上述能力

## 关键设计决策

- 第 1 轮范围固定为 `USDJPY + 1m + UTC + 研究优先`
- 数据管理统一采用 `Bronze / Silver / Gold`
- 外部数据源必须通过 provider 抽象接入，第 1 轮优先 `文件导入`
