...
- 本项目统一使用仓库根目录下新建的 `.venv` 作为测试虚拟环境；测试、回归、依赖安装默认使用 `.\\.venv\\Scripts\\python.exe`
- `.venv` 由 `Python 3.14` 创建，并按 `pyproject.toml` 安装 `-e .[api,backtest,db,dev,orchestration,research]`
- 不再使用与项目技术栈不一致的旧环境执行正式测试
- 每进入下一步开发前，必须先把当前已存在测试完整跑完，并确认结果为绿灯
- 新增功能、修 bug、重构或补文档导致行为变化时，必须先补齐相关自动化测试，再继续推进下一项任务
- 如果测试未跑、未通过、或仅跑了局部用例，不视为当前阶段完成
...
- 数据接入：第 1 轮主路径为 `Massive/Polygon 免费档 API 小样本`
- 文件导入：仅用于真实样本回放与离线复现
- 注册表：开发态允许 `sqlite`，设计兼容 `PostgreSQL`
- 回测：研究层完整，订单级先骨架
- API：只做最小 FastAPI 骨架
- 测试：默认基于真实 API 获取后固化的小样本，不使用生成假数据作为正式测试依据
...
- 本地 git 仓库已初始化
- 后续每次有效修改后，本规则立即生效
- 默认测试命令：`.\\.venv\\Scripts\\python.exe -m unittest discover -s tests`
